...
export function renderEnvironmentsPage(opts: EnvPageOptions): HTMLElement {
  ...
  const attachEvents = () => {
    ...
    // Add environment
    el.querySelector('#btn-add-env')?.addEventListener('click', () => {
      const name = prompt('Environment name:');
      if (!name?.trim()) return;
      const newEnv: Environment = { id: generateId(), name: name.trim(), variables: [], isActive: false };
      envs.push(newEnv);
    ...
    el.querySelector('#btn-dup-env')?.addEventListener('click', () => {
      const env = envs.find((e) => e.id === selectedEnvId);
      if (!env) return;
      const name = prompt('New environment name:', `${env.name} (Copy)`);
      if (!name?.trim()) return;
      const copy: Environment = { ...env, id: generateId(), name: name.trim(), isActive: false, variables: env.variables.map((v) => ({ ...v })) };
      envs.push(copy);
    ...
    el.querySelector('#btn-del-env')?.addEventListener('click', async () => {
      ...
      if (!confirmed) return;
      opts.onDelete(env.id);
      envs = envs.filter((e) => e.id !== env.id);
      if (envs[0]?.isActive === false && envs.length) envs[0].isActive = true;
    ...
    // Add variable
    el.querySelector('#btn-add-var')?.addEventListener('click', () => {
      const env = envs.find((e) => e.id === selectedEnvId);
      if (!env) return;
    ...
    el.querySelectorAll('.env-var-row').forEach((row) => {
      ...
      row.querySelectorAll('[data-var-field]').forEach((input) => {
        input.addEventListener('blur', () => {
          ...
          env.variables[idx] = { ...env.variables[idx], [field]: (input as HTMLInputElement).value };
          opts.onSave(env);
        });
      });

      row.querySelector('[data-del-var]')?.addEventListener('click', () => {
    ...
    <div style="display:grid;grid-template-columns:220px 1fr;gap:var(--space-6)">
      ...
      <div>
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          ${envs.map((e) => `
            <div data-env-id="${e.id}" style="display:flex;align-items:center;gap:var(--space-2);padding:var(--space-2) var(--space-3);border-radius:var(--radius-md);cursor:pointer;background:${e.id === selectedId ? 'var(--color-primary-light)' : 'transparent'};color:${e.id === selectedId ? 'var(--color-primary)' : 'var(--color-text-primary)'}">
          ...
          <div class="card">
            <div class="card-header">
              ...
              <div style="display:flex;gap:var(--space-2)">
                ${!selected.isActive ? `<button class="btn btn-secondary btn-sm" data-set-active="${selected.id}">${Icons.checkCircle({ size: 14 })} Set Active</button>` : `<span class="badge badge-green">${Icons.check({ size: 12 })} Active</span>`}
                ...
                <tbody>
                  ${selected.variables.map((v, i) => `
                    <tr class="env-var-row" data-idx="${i}">
                      ...
                      <td><input class="input" type="text" value="${escapeHtml(v.value)}" placeholder="value" data-var-field="value" style="font-family:var(--font-mono);font-size:var(--text-xs)"/></td>
